...
Также вакансии из Telegram-каналов отправляются автоматически ежедневно в **19:00 UTC**.
...
Формат выдачи вакансий:
- основной блок: вакансии по Санкт-Петербургу;
- дополнительный блок внизу: `удаленная работа` (вакансии с пометкой «удаленная работа/удалённая работа» независимо от города).
...
Используется `APScheduler` с `CronTrigger`.

- Почтовый дайджест: будни `mon-fri`, минуты `:00` (часы берутся из `SCHEDULE_HOURS`).
- Дайджест вакансий из Telegram-каналов: ежедневно в `19:00 UTC`.
...
Для более узкой фильтрации можно указывать часть слова. Например, `TELEGRAM_VACANCY_BANNED_WORDS=врач,водитель,агент,терапевт,диспетчер,медицин` отфильтрует вакансии с фрагментом `медицин` в названии (например: `медицины`, `медицинской`, `медицинскому`).
